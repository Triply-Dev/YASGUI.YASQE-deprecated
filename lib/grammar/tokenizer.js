...
CodeMirror.defineMode("sparql11", function(config, parserConfig) {
  ...
  var LINE_BREAK = "\n";
  var STRING_LITERAL1 = "'(([^\\x27\\x5C\\x0A\\x0D])|" +
    ECHAR +
    "|" +
    unicode +
    ")*'";
  var STRING_LITERAL2 = '"(([^\\x22\\x5C\\x0A\\x0D])|' +
    ECHAR +
    "|" +
    unicode +
    ')*"';
  ...
  function tokenBase(stream, state) {
    function nextToken() {
      ...
      if (state.inLiteral) {
        ...
        consumed = stream.match(
          stringLiteralLongRegex[state.inLiteral].contents.regex,
          true,
          false
        );
        if (consumed && consumed[0].length == 0) {
          ...
          consumed = stream.match(
            stringLiteralLongRegex[state.inLiteral].closing.regex,
            true,
            false
          );
          closingQuotes = true;
      ...
      for (var quoteType in stringLiteralLongRegex) {
        consumed = stream.match(
          stringLiteralLongRegex[quoteType].quotes.regex,
          true,
          false
        );
        if (consumed) {
          ...
          if (topSymbol == token) {
            if (state.inPrefixDecl) {
              if (topSymbol === "PNAME_NS" && tokenOb.text.length > 0) {
                ...
                state.currentPnameNs !== undefined && tokenOb.text.length > 2
              ) {
                state.prefixes[state.currentPnameNs] = tokenOb.text.slice(
                  1,
                  -1
                );
            ...
            if (
              ...
              if (colonIndex >= 0) {
                ...
                if (
                  !state.prefixes[prefNs] && ["bif", "xsd"].indexOf(prefNs) < 0
                ) {
                  state.OK = false;
                  recordFailurePos();
